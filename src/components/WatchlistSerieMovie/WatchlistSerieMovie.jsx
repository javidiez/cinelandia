...
export const WatchlistSerieMovie = () => {
    ...
    return (
        <>
            ...
            <div className="mt-4 bloque-card-mobile-watchlist fade-in ms-5">
                <div className="swiper-container-watchlist">
                    <h2 className="ms-3 fs-1 text-light">Películas</h2>
                    <div className="swiper-wrapper-watchlist scrollableDiv-watchlist gap-5 mb-5">
                        {store.watchlist && store.watchlist.length > 0 ? (
                            store.watchlist.map((fav, index) => {
                                ...
                                const isUpcoming = releaseDate > today ? "Próximo estreno" : "";
                                return (
                                    <div key={index} className='fade-in shadow-watchlist'>
                                        <FilmCard
                                            key={fav.id}
                                            ...
                                            proxEstreno={isUpcoming}
                                        />
                                        <button onClick={() => actions.deleteFavouriteMovie(fav)} className="btn"><i className="fa-solid fa-trash"></i></button>
                                    </div>
                                )
                            })) :
